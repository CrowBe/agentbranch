...
**The wedge is the hosted graphical flow.** The eval tooling that exists elsewhere is CLI-bound (Anthropic's skill-creator runs trigger evals as a terminal meta-skill), so for the builder who will never run a CLI, a hosted surface — evaluations launched from the hero's chips, landing as Insights — is the only validation on offer. That gap, plus **honest cross-runtime results** ([#116](https://github.com/CrowBe/agentbranch/issues/116)) and **trust-gated distribution** ([§9.1](#91-skill-tap--moderation-before-distribution)), is the defensible ground; a triggering eval alone is table stakes the CLI tools already provide. **Teams** (shared libraries, private validation gates, seats) are the named later revenue layer, not a v1 audience.
...
| **Safety rating** | *User-facing term* for the persisted result of a **safety review** run ([§9.1](#91-skill-tap--moderation-before-distribution)): verdict (passed / needs review / blocked) + per-class risk scores + Insight, pinned to the skill version reviewed. **Opt-in and manual, never automatic** — offered any time for an unrated version and as an optional, non-gating step of "Set as main version". User-triggered runs spend `account` (`safety-review` capability); the publication gate's runs spend `platform`. |
...
- **`publications`** — public addresses for validated skill versions ([§9.1](#91-skill-tap--moderation-before-distribution)): publisher, `owner/skill-name` slug, trust tier (private / community / reviewed), pinned append-only `skill_version_id`, content hash, and the gate run + harness version that reviewed the exact artifact. A changed version means a different hash and a new gate run before community/reviewed visibility.
...
- **`safety_ratings`** — recorded **safety ratings** ([§9.1](#91-skill-tap--moderation-before-distribution)): verdict + per-class scores + Insight for one safety-review run, with the same nullable `skill_version_id` / `harness_version_id` pins (`onDelete: SetNull`) as the other evaluation records. The version pin is load-bearing for the opt-in flow — "does this version carry a rating?" is a `latestForVersion` read, so an unrated head is what triggers the offer.
...
**Build order after the v1 slice** — with the cross-runtime validation engine in place, the differentiators continue with the **draft/promote UI** ([#129](https://github.com/CrowBe/agentbranch/issues/129)) riding the already-landed branch substrate, then **tap moderation** ([#133](https://github.com/CrowBe/agentbranch/issues/133) steps 1–3). Primitives phases 4–6 ([#126](https://github.com/CrowBe/agentbranch/issues/126)) wait for a demand signal. The **harness improvement loop** ([#118](https://github.com/CrowBe/agentbranch/issues/118)) is built end-to-end (see [*Harness improvement loop*](#harness-improvement-loop-admin) below); its recommendations sharpen as the corpus of real user runs grows.
...
- **Baseline skill corpus** ([#149](https://github.com/CrowBe/agentbranch/issues/149)) *(built)* — a repo-tracked set of 10 curated skills for common use cases, with source prompts, prompt batteries, provenance, and content hashes. It *is* the frozen regression benchmark's set (below), feeds the triggering eval's distractor library, and is ready to seed Templates.
...
- **Skill tap** (publish + Skill library) *(moderation gate + publication domain + distribution read models built; the public tap repo, bot publish pipeline, and workspace surfaces remain)* — a public installation source other tools add once and install from. Moderation must precede distribution — full design in [§9.1](#91-skill-tap--moderation-before-distribution) ([#133](https://github.com/CrowBe/agentbranch/issues/133)).
...
The in-app half is built; the public half is not ([#133](https://github.com/CrowBe/agentbranch/issues/133)). Built: the full moderation gate (lint policy rules, adversarial negative battery, `safetyReviewCapability`), the publication domain (`publications`, [§6](#6-data-model-sketch)), and the pure distribution read models — the tap marketplace manifest renderer and the Skill-library feed (`/api/skill-library`, Templates as a view over the reviewed tier). Not built: the public tap repo itself, the bot publish pipeline (PR flow, open-source lint CI, hosted-gate status checks, auto-merge, revert path), a publish route/UI invoking `publishSkillVersion`, and the workspace surfaces consuming the feed. **This design gates the tap: no tap code lands until the moderation system exists** — shipping a distribution source without it is a reputational one-way door. "Tap" (an installation source you add once, then install from by name) is the internal term; user-facing copy says **Publish** / **Skill library**. A tap is *distribution*, not deploy: a validated skill gets a public address, nothing runs.
...
**The gate is the seam** — agent.branch is a validation tool that also distributes, not a file host bolting moderation on. Publication reuses the seam: **skill lint** carries the static policy rules (shell-exec patterns, credential paths, fetch-and-follow indirection, obfuscation), reading the whole folder when reference-file contents are provided; the **triggering eval**'s negative battery includes fixed adversarial prompts, making it the trigger-hijack scanner; the **test run** catches behaviour the text hides (exfiltration-shaped tool calls show in the transcript); and a new **`safetyReviewCapability`** — an LLM-judge evaluation capability, caller-tagged (`platform` when the gate runs it) — reads the full folder and scores injection/exfiltration/deception. The judge is **structurally untrusted**: it reads the skill strictly as data, and its verdict is a backstop, never the sole gate.

**Safety ratings are opt-in — manual, never automatic.** The same `safetyReviewCapability` doubles as a user-facing pre-check: a **safety rating** the user runs on demand. A rating pins the version it reviewed; versions are append-only, so a rating stays true forever and a new revision is simply *unrated* — the scan is offered any time for an unrated version (the hero's **Safety** chip) and as an **optional step of "Set as main version"** (skippable, and a worrying verdict only asks — it never gates promote; gating belongs to publication). Because the user asks for it, the run is `account`-tagged against the `safety-review` gated capability (Pro tier) — nobody burns credits on a rating they didn't request — while the gate's own runs stay `platform`-tagged; the capability takes the tag from its caller. A rated version never re-spends: reopening the rating and switching Insights/breakdown render the stored record. Ratings persist as `safety_ratings` ([§6](#6-data-model-sketch)), pinned to skill version + harness version like every evaluation record.
...
**Build order — moderation leads, distribution follows:** the moderation layers (static lint policy, adversarial negative battery, `safetyReviewCapability`), the publication domain (pinned version, slug, tier, hash, rate limit), and the distribution read models (tap marketplace manifest, Skill-library feed with Templates as a view over the reviewed tier) all run today. What remains is the outward-facing half: the public tap repo + bot (PR flow, open-source lint CI, hosted-gate checks, auto-merge, revert path), the publish route/UI, and the workspace Skill-library/Templates surfaces.
