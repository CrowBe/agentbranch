...
- **Skill tap** (publish + Skill library — an installation source other tools add once and install from) — distribution gated on the seam's own checks; design and the moderation system that must precede it live in **`SKILL_TAP.md`**. User-facing copy says "Publish" / "Skill library", never "tap".
