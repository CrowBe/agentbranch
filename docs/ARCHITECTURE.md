...
| **Response schema** | The first equipment primitive beyond Skill ([§9.2](#92-equipment-primitives--composition-broadening-beyond-skill)): a structured output definition — a JSON Schema document — with a lossless source model and pure offline lint. Analysis-only (no evaluator); its schema subset also validates tool-contract examples and test-run calls. |
