...
- **Branching iteration** — move versioning from a linear latest-head model to safe-space iteration: draft changes happen on a branch, evaluation runs against that branch, and the blessed version only moves when the user promotes the branch. The implementation should borrow git's shape without forcing git vocabulary onto the SMB-facing UI. Tracked with the primitives expansion ([#126](https://github.com/CrowBe/agentbranch/issues/126)) — the iteration substrate lands first.
