...
**Build order after the v1 slice** — with the cross-runtime validation engine in place, the differentiators continue with the **draft/promote UI** ([#129](https://github.com/CrowBe/agentbranch/issues/129)) riding the already-landed branch substrate, then **tap moderation** ([#133](https://github.com/CrowBe/agentbranch/issues/133) steps 1–3). Primitives phases 4–6 ([#126](https://github.com/CrowBe/agentbranch/issues/126)) wait for a demand signal; the harness loop's corpus-mining phases ([#118](https://github.com/CrowBe/agentbranch/issues/118)) wait for a real corpus of user runs — with two pieces pulled forward: **harness-manifest stamping** ([#120](https://github.com/CrowBe/agentbranch/issues/120), cheap now, impossible to retrofit) and the **baseline skill corpus** ([#149](https://github.com/CrowBe/agentbranch/issues/149), below).
...
- **Baseline skill corpus** ([#149](https://github.com/CrowBe/agentbranch/issues/149)) — a repo-tracked set of 10 curated skills for common use cases, with source prompts, prompt batteries, provenance, and content hashes. It is the raw material for the frozen regression benchmark, feeds the triggering eval's distractor library, and is ready to seed Templates.
...
Designed, not built ([#118](https://github.com/CrowBe/agentbranch/issues/118)). The harness improvement loop is the admin-only feedback system that reads stored evaluation records across sessions and recommends changes to the validation harness itself. It is the first planned use of the skill-analysis seam where `Input` is not a single `Skill`: input is a **cohort of evaluation records** (triggering eval + test run records, pinned to skill versions and harness versions), artifact is a **harness-recommendation report**, and renderer is an **admin report surface**. If recommendations are model-written, the capability is an **evaluation capability** (`defineEvaluation`) and calls the model gateway with a `platform` accounting tag; static correlation-only recommendations can be an analysis capability.
...
The only new persistence surface is an **admin aggregate-read port** over evaluation records. User-scoped reads remain the default and ownership stays enforced in query for ordinary product surfaces; the aggregate read is gated by `isAdmin` and the same allowlist posture as the model console. The default aggregate output is outcomes and features — pass/fail, finding ids, case expectations, rationales, timestamps, skill and harness ids — not raw `SKILL.md` content or prompts. Reading content requires an explicit consent boundary because cross-user content is sensitive even when the goal is product improvement.

The first recommendation tier is deliberately auditable: correlate lint/static features with evaluation failures and propose lint-rule or weighting changes, each traceable to its evidence records. A human applies any change as an ordinary reviewed diff. The regression benchmark is the guardrail: recommendations are not "better" until the frozen benchmark shows they improve outcomes without regressing known-good cases. This is the cross-session cousin of **eval feedback**: eval feedback helps Claude revise one skill in the build loop; the harness improvement loop helps admins improve the evaluators and prompts that produce those results.
