...
**Build order after the v1 slice** — the two differentiators lead: **cross-runtime validation** first ([#116](https://github.com/CrowBe/agentbranch/issues/116) — the sharpest claim, and mostly a provider swap through the existing router), with the **draft/promote UI** ([#129](https://github.com/CrowBe/agentbranch/issues/129)) riding the already-landed branch substrate alongside; **tap moderation** ([#133](https://github.com/CrowBe/agentbranch/issues/133) steps 1–3) follows. Primitives phases 4–6 ([#126](https://github.com/CrowBe/agentbranch/issues/126)) wait for a demand signal; the harness loop's corpus-mining phases ([#118](https://github.com/CrowBe/agentbranch/issues/118)) wait for a real corpus of user runs — with two pieces pulled forward: **harness-manifest stamping** ([#120](https://github.com/CrowBe/agentbranch/issues/120), cheap now, impossible to retrofit) and the **baseline skill corpus** ([#149](https://github.com/CrowBe/agentbranch/issues/149), below).
...
- **Baseline skill corpus** ([#149](https://github.com/CrowBe/agentbranch/issues/149)) — ~10–20 curated skills for common use cases, authored *outside* the platform with the best external tooling (Anthropic's skill-creator) so the bar is honest, each admitted only after passing our own lint + triggering eval. Triple duty: raw material for the frozen regression benchmark, a distractor-library upgrade for the triggering eval, and Templates seeds.
