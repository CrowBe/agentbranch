...
**Build order (when greenlit) — moderation leads, distribution follows:** static lint policy is the first layer and runs today; the remaining layers land in order: 1. adversarial battery → 2. `safetyReviewCapability` → 3. publication domain (pinned version, slug, tier, hash, rate limit) → 4. tap repo + bot (PR flow, open-source lint CI, hosted-gate checks, auto-merge, revert path) → 5. reviewed tier + Skill library surface (Templates becomes a view over the reviewed tier).
