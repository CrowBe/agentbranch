...
| **concept-library** | `conceptLibrary`, `CONCEPT_GLOSSARY_TERMS`, `Concept`, `DefinitionConcept`, `DecisionAidConcept`, `ConceptClaim`, `ConceptCitation` + types | — | real (repo-tracked, content-hashed thin concept kernels; every bounded claim cites a primary source and glossary terms are checked against `CONTEXT.md`) |
...
| **build-loop** | `runBuildLoop`, `buildTools`, `BuildToolName`, `BuildLoopEvent`, `formatConceptContext`, `ConceptGlossary`, `formatTestRunFeedback`, `formatTriggeringEvalFeedback`, `runResponseSchemaLoop`, `responseSchemaTools`, `RESPONSE_SCHEMA_AUTHORING_PROMPT`, `formatResponseSchemaLintFeedback` | — (consumes `ModelGateway`) | real (concept interrogation is a pure, delimited evidence formatter; every authoring prompt answers from its cited kernel and glossary without interviewing or mutating an artifact) |
