...
| `ai/codex-model-calls.ts` | `RawModelCalls` | Codex CLI dev rung: subscription-authenticated `classify`/`generate` through `codex exec` native output schemas in an ephemeral read-only scratch run, loaded only behind a development-only boundary so its filesystem runner is absent from production route traces. Each call gets isolated `HOME` and `CODEX_HOME` directories containing only a mode-0600 copy of the logged-in runtime's `auth.json`; global config, cache, skills, rules, and session state are not exposed through CLI startup. The source login must be current: token refreshes written inside a disposable copy are intentionally not synced back to global auth, so an expired or already-consumed refresh token fails closed and requires the operator to log in again. An environment allowlist, user-config/rule exclusion flags, disabled shell/exec/browser/app/delegation tools, and a pure-inference prompt further narrow the boundary. This prevents intended CLI/model access to global developer state, but it is not an OS-level filesystem sandbox against a compromised CLI binary. Process-group timeout escalation, output/concurrency bounds, and whole-run cleanup contain each call. Agent primitives are honestly unavailable because caller-owned gateway tools have no in-process bridge. |
