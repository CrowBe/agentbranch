...
| `ai/codex-model-calls.ts` | `RawModelCalls` | Codex CLI dev rung: subscription-authenticated `classify`/`generate` through `codex exec` native output schemas in an ephemeral read-only scratch run. It inherits only an allowlist of process environment (including the real logged-in `CODEX_HOME`), excludes user config/rules, disables shell/exec/browser/app/delegation tools, and gives the model a pure-inference prompt. This prevents model-initiated host reads; it is not a host-level filesystem sandbox because the Codex runtime itself still reads authentication. Process-group timeout escalation, output/concurrency bounds, and cleanup contain each call. Agent primitives are honestly unavailable because caller-owned gateway tools have no in-process bridge. |
