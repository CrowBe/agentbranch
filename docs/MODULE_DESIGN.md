...
| Agent-configuration import preview | analysis | `agent-configuration-import` | validated source snapshot + runtime adapter probes → redacted runtime-neutral snapshot with deterministic detected-adapter provenance, evidence, warnings, unsupported content, and collisions (pure) | `preview` | real |
...
| **eval-comparison** | `compareEvalRuns`, comparison artifact + refusal-reason types | — (consumes `EvalRunRepository`) | real · user-scoped derived read model · exact paired-case comparability gates · deterministic case-cluster bootstrap 95% interval; never persisted |
...
|---|---|---|
| `agent-configuration-import/runtime-adapters.ts` | `AgentConfigurationImportAdapter` | deterministic Claude Code, Codex, OpenClaw, and open `.agents` probes/classifiers; parsing only, source evidence required |
| `agent-configuration-import/effective-configuration-adapters.ts` | effective-configuration resolution policy | translates Claude Code, Codex, OpenClaw, and open `.agents` instruction order, override precedence, declarations, and explicit source relationships into the runtime-neutral domain rule vocabulary |
| `agent-configuration-import/{source-snapshot,archive}.ts` | repository/local-archive ingestion boundary | clones supplied repository blobs and parses ZIP/TAR/TAR.GZ in memory; rejects unsafe paths, links/special entries, ambiguous collisions, and resource-limit violations before adapters run |
| `memory/agent-configuration.memory-repository.ts` | `AgentConfigurationRepository` | offline adapter; preserves complete snapshots, including detected-adapter provenance, across immutable main versions and one replaceable configuration-wide draft |
...
| `ai/codex-model-calls.ts` | `RawModelCalls` | Codex CLI dev rung: subscription-authenticated `classify`/`generate` through `codex exec` native output schemas, loaded only behind a development-only boundary so its filesystem runner is absent from production route traces. Each adapter instance lazily creates one mode-0700 sanitized session `HOME`/`CODEX_HOME`, copies only the source runtime's `auth.json` at mode 0600, and serializes calls at a hard maximum concurrency of one. CLI-written auth rotation and cache persist across calls inside that session, while global config, cache, skills, rules, and session state are never copied and session state is never synced back. A process restart creates a new sanitized session from source auth; if that source login is stale, the operator may need to run `codex login` again. Each call uses and cleans its own scratch/schema/output directory. An environment allowlist, user-config/rule exclusion flags, disabled shell/exec/browser/app/delegation tools, and a pure-inference prompt further narrow the boundary. This prevents intended CLI/model access to global developer state, but it is not an OS-level filesystem sandbox against a compromised CLI binary. Process-group timeout escalation and output bounds contain each call. Agent primitives are honestly unavailable because caller-owned gateway tools have no in-process bridge. |
...
- `app/api/eval-comparison/route.ts` — authenticated, user-scoped derived
  comparison addressed by ordered left/right run ids and method version. It
  stores no comparison row and exposes no prompts.
