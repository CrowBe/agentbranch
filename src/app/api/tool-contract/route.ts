...
const requestSchema = z.object({
  /** The raw tool-contract document (JSON text). */
  document: z.string().min(1, "Send a tool contract document to check."),
  surface: z.enum(["insights", "breakdown"]).default("insights"),
});

/** Quality-check a tool-contract document — pure analysis, runs offline. */
export async function POST(request: Request): Promise<Response> {
  ...
  if (identity.value === null) {
    return Response.json({ error: "Sign in to check a tool contract." }, { status: 401 });
  }
  ...
  const source = parseToolContract(parsed.data.document);
  if (!source.ok) return invalidRequestResponse(source.error.message);

  const result = await runCapability(toolContractCapability, parsed.data.surface, source.value);
