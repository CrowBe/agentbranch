...
import { lintCapability } from "@/modules/lint";
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (identity.value === null) {
    return Response.json({ error: "Sign in to lint a skill." }, { status: 401 });
  }
  ...
  const result = await runCapability(lintCapability, surface.data, skill);
