...
export async function* runSubagentDefinitionLoop(input: SubagentDefinitionLoopInput, gateway: ModelGateway, userId: UserId): AsyncGenerator<SubagentDefinitionLoopEvent> {
  ...
  for await (const part of opened.value) {
    ...
    else if (part.kind === "tool-result") { yield { event: "tool", data: { name: part.tool, phase: "result" } }; const out = part.output as Record<string, unknown> | null; if (part.tool === "write_subagent_definition" || part.tool === "edit_subagent_definition") attemptedMutation = true; if (out && part.tool === "write_subagent_definition" && typeof out.content === "string") { const parsed = parseSubagentDefinition(out.content); if (parsed.ok) { hasDraft = true; yield { event: "subagent-definition", data: { source: parsed.value } }; } } else if (out && part.tool === "edit_subagent_definition" && typeof out.oldStr === "string" && typeof out.newStr === "string") yield { event: "subagent-definition-edit", data: { oldStr: out.oldStr, newStr: out.newStr } }; }
    else if (part.kind === "finish") { if (attemptedMutation && !hasDraft) yield { event: "error", data: { message: "No subagent definition was created. Retry to create a new draft." } }; yield { event: "done", data: { finishReason: part.finishReason } }; }
